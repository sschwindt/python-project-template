...
def gradually_varied_profile(
    Q: float, b: float, n: float, S0: float, z: np.ndarray, dx: float, y0: float
) -> np.ndarray:
    ...
    y = float(y0)
    ys = []
    for _ in range(len(z)):  # one explicit step per node of the bed profile

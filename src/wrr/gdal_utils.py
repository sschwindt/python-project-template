...
def warp_resample(
    src_path: str, dst_path: str, dst_crs: str, res: float, resampling="bilinear"
) -> None:
    ...
    with rasterio.open(src_path) as src:
        ...
        profile.update(
            crs=dst_crs,
            transform=transform,
            height=int(src.height * src.res[0] / res),
            width=int(src.width * src.res[1] / res),
        )
...
def rasterize_polygons(polygons, values, template_path: str, out_path: str) -> None:
    ...
    out = features.rasterize(
        [(mapping(geom), val) for geom, val in zip(polygons, values, strict=False)],
        out_shape=shape,
        transform=transform,
        fill=0,
        dtype="int16",
...
def sample_profile_from_raster(
    raster_path: str, line: LineString, npts: int
